    ...
    try {
      ...
      if (filters && filters.length > 0) {
        filters.forEach((filter) => {
          ...
          if (op === 'eq') {
            if (col === 'phone') {
              whereClauses.push(`RIGHT(REPLACE(phone, ' ', ''), 10) = RIGHT(REPLACE(?, ' ', ''), 10)`);
      ...
      for (const row of rows) {
        if (table === 'rides') {
          ...
          if (!row.otp && row.status !== 'completed' && row.status !== 'cancelled') {
            const autoOtp = String(Math.floor(1000 + Math.random() * 9000));
            await db.query(
              `INSERT INTO ride_otp (ride_id, otp, is_verified, generated_at) VALUES (?, ?, 0, NOW()) ON DUPLICATE KEY UPDATE otp = VALUES(otp)`,
              [row.id, autoOtp]
            ).catch(() => {});
            row.otp = autoOtp;
          } else {
      ...
      if (table === 'driver_profiles') {
        ...
        if (driverUuid) {
          ...
          if (dp && dp.id) {
            ...
            // Extract and update live location
            if (row.current_latitude !== undefined || row.current_longitude !== undefined) {
              const lat = row.current_latitude;
              const lng = row.current_longitude;
              if (lat !== undefined && lng !== undefined) {
                // Ensure a row exists in driver_live_location or update it
                await db.query(
        ...
        // Remove document and location keys from payload so we don't try to update them in driver_profiles table
        ...
        delete row.current_latitude;
        delete row.current_longitude;
      }

      const setClauses = [];
      const queryParams = [];

      // Date/datetime columns: convert empty strings to NULL, ISO strings to MySQL format
      const dateColumns = ['date_of_birth', 'dob', 'license_expiry'];
      const datetimeColumns = ['updated_at', 'created_at', 'last_login', 'accepted_at', 'started_at', 'completed_at', 'cancelled_at'];
      for (const key of Object.keys(row)) {
        ...
        if (datetimeColumns.includes(key)) {
          ...
          if (val === '' || val === undefined) {
            row[key] = null;
          } else if (typeof val === 'string' && (val.includes('T') || val.endsWith('Z'))) {
            // Convert ISO 8601 to MySQL DATETIME format: 'YYYY-MM-DD HH:MM:SS'
            const d = new Date(val);
            if (!isNaN(d.getTime())) {
              row[key] = d.toISOString().slice(0, 19).replace('T', ' ');
            }
          }
        }
      }
